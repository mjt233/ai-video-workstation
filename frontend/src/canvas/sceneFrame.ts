...
/** 新场景帧定义（stage.json 单帧结构；基础场景为空表示独立场景图帧：纯图片、无合成来源） */
export interface SceneFrameBody {
  /** 基础场景引用（场景名/标签 或 prev）；为空时该帧为独立场景图（纯图片） */
  基础场景: string
  /** 登场角色列表（空基础场景时恒为空） */
  登场角色: string[]
  /** 生成提示词（空基础场景时恒为空） */
...
/**
 * 从生成节点的输入推导新场景帧的 stage.json 定义。
 * 基础场景优先取输入图中的 `assert/stage/{场景}/{标签}`（含变体路径），
 * 否则复用现有帧中第一个非空基础场景；登场角色取自 `assert/character/{角色}` 输入（去重）。
 * 无可用基础场景时返回「独立场景图帧」（基础场景、登场角色与 prompt 均为空）：
 * 允许直接把当前产物作为分镜场景图，不要求先有基础场景引用。
 ...
 * @returns 新帧定义（始终非空：无基础场景时为基础场景为空的独立场景图帧）
 */
export function deriveStageFrameBody(
  ...
  // 无可用基础场景：退化为独立场景图帧（纯图片，角色与 prompt 恒为空）
  if (!baseScene) return { 基础场景: '', 登场角色: [], prompt: '' }
