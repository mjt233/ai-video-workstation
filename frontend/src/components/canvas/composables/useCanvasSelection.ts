...
export function useCanvasSelection(options: UseCanvasSelectionOptions) {
  ...
  /** 勾选/取消勾选单个分组（Ctrl 单击标题条语义） */
  function toggleSelectGroup(groupId: string): void {
    const next = selectedGroupIds.value.includes(groupId)
   ...
   *
   * 语义（FR-9.1 / FR-9.2）：
   * - 仅分组被选中 → 「解散分组」（只删框，组内节点保留）；
   * - 含节点选中 → 删除节点并连带解散选中分组，一次确认。
   */
  async function deleteSelected(): Promise<void> {
    ...
    if (nodeIds.length === 0) {
      const ok = await confirm({
        ...
        content: groups.length === 1
          ? `确定解散分组「${groups[0]?.name ?? ''}」？组内节点将保留。`
          : `确定解散选中的 ${groups.length} 个分组？组内节点将保留。`,
        confirmText: '解散',
        confirmColor: 'error',
      })
      if (!ok) return
    ...
    if (groupIds.length > 0) {
      content = `确定删除选中的 ${nodeIds.length} 个节点和 ${groupIds.length} 个分组？`
    } else if (nodeIds.length === 1) {
      content = `确定删除节点「${nodes[0]?.name ?? ''}」？`
    } else {
      content = `确定删除选中的 ${nodeIds.length} 个节点？`
    }
    const ok = await confirm({ title, content, confirmText: '删除', confirmColor: 'error' })
    if (!ok) return
    if (nodeIds.length === 1 && groupIds.length === 0) {
      store.removeNode(nodeIds[0])
    } else {
      store.removeNodes(nodeIds, groupIds)
    }
    selectedNodeIds.value = []
    selectedGroupIds.value = []
  }
