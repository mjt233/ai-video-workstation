...
- **工作流完成通知气泡与任务管理器抽屉**：工作流任务（生成图片 / 生成视频 / TTS / **文本生成**）终态由统一任务广播（`taskSocket.onTaskUpdate`）驱动，右下角弹**独立气泡卡片**（产物预览、点击放大、**固定 30s 自动关闭**、最多 3 张、`×` 手动关闭、`TransitionGroup` 进出场过渡）——配置项有意不提供，卡片纯内存不持久化；**任何工作流终态还会自增 `canvas/notify.ts` 的 `workflowFinishedTick`**，`TaskHistoryPanel` 自己监听它重新拉首批（历史在 SQLite，不会自动感知新任务；用户已滚动离开顶部时不打断）。任务管理器为**右侧 `v-navigation-drawer`（temporary 浮层，不挤压画布，ESC/遮罩关闭）**，含「进行中 / 历史」两个页签；「历史」只按**时间范围 + 状态**筛选（「项目」筛选已移除）、用 Vuetify `<v-infinite-scroll>` **组件**滚动加载（首批 30 条，注意 Vuetify 4 已无同名指令、intersect 模式下 `#error` 槽不可省），行内提供产物缩略图与同一个放大预览对话框，任务日志在「任务详情」对话框（`CanvasNodeLogDialog`）里看——**行内不展开日志**，以保证两行定高。**分工**：气泡负责"不用操作就能看到"，任务管理器负责"回看与排障"。改这块前先读 [./docs/canvas/notification.md](./docs/canvas/notification.md) 与 [./docs/task-manager/ui.md](./docs/task-manager/ui.md)；新增工作流失败原因展示须经 `workflowExecutor.finish(taskId, status, error)` 透传（否则终态广播不带原因）。
- **工作流类型与产物形态**：系统工作流类型见 `server/src/workflows/types.ts` 的 `WorkflowTypeId`（`text-to-image` / `image-edit` / `tts-voice-design` / `tts-voice-clone` / `image-to-video` / **`text-generation`**），新增类型必须同步：`workflows/vars.ts` 的 vars interface、`providers/custom/types.ts` 的 `CUSTOM_WORKFLOW_TYPES`、前端 `utils/workflow-types.ts`（标签/颜色/兜底清单）、自定义脚本类型提示 `utils/custom-provider.ts`（`PARAM_INTERFACES` + `PARAM_INTERFACE_NAMES` + 模板）、Bridge 推导 `workflows/bridge-derive.ts`（如需）。**产物分两种形态**：媒体类走 `WorkflowOutput` 的 `download|fetch|body`（引擎下载/解码写 `assert/{outputPath}`，需要 `params.outputPath`）；**文本类（`text-generation`）走新增的 `{ type: 'text', text }`**——`/api/workflow/run` 对该类型放宽 `outputPath` 必填，引擎不写 `assert/`、不归档资产历史，改为把文本写回画布节点 `config.output` + 追加 `config.outputHistory`（`canvas/text-result.ts`，CAS + 路径锁 + 版本冲突重试；历史规则在 `canvas/text-history.ts` 与 LLM 会话落盘共用）。自定义服务商脚本可直接 `return { isFinish: true, text: '…' }`（与 `outputs` 二选一，同时存在以 `text` 为准；非文本类型返回 `text` 只告警忽略）；ComfyUI Bridge 的文本产物按 `output-files` 的 `.txt/.md/.json` 扩展名识别后下载解码。文本任务无 `outputPath`，画布侧靠任务 `result` 里的 `{ text, patch, prevRev, rev }` 做版本对齐采纳（`AssetCanvas.adoptTextRunResult`，与 LLM 会话终态 `adoptLlmResult` 同一套语义）。
- **画布节点媒体输入预览规则**：任何节点主体需要展示已连接的媒体输入（图片/音频/视频）时，必须复用生成图片/生成视频节点使用的统一输入预览组件 `frontend/src/components/canvas/editors/CanvasInputPreview.vue`（现有实例：AI文本生成节点、文本生成节点），禁止为节点单独实现一套预览/徽标 UI。具体做法：
