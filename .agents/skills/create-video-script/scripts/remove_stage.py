...
示例:
    python scripts/remove_stage.py -e 1 1          # 移除第1集分镜1的最后一条场景定义
    python scripts/remove_stage.py -e 1 1 0        # 移除第1集分镜1的第一条场景定义
    python scripts/remove_stage.py -e 1 2 1        # 移除第1集分镜2的第二条场景定义
"""

import argparse
import sys
from _common import DEFAULT_PROJECT, DEFAULT_EPISODE, read_json, write_json, get_stage_json_path
...
def main():
    ...
    if not data:
        print(f"⚠️  第{episode}集 分镜 {args.shot} 的 stage.json 为空，无需移除。", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"✅ 已移除第{episode}集 分镜 {args.shot} 的第 {idx} 条场景定义:")
