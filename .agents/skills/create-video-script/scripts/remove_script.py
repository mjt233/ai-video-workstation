...
示例:
    python scripts/remove_script.py -e 1 1          # 移除第1集分镜1的最后一条台词
    python scripts/remove_script.py -e 1 1 0        # 移除第1集分镜1的第一条台词
    python scripts/remove_script.py -e 1 2 1        # 移除第1集分镜2的第二条台词
"""

import argparse
import sys
from _common import DEFAULT_PROJECT, DEFAULT_EPISODE, read_json, write_json, get_script_json_path
...
def main():
    ...
    if not data:
        print(f"⚠️  第{episode}集 分镜 {args.shot} 的 script.json 为空，无需移除。", file=sys.stderr)
        sys.exit(1)
    ...
    print(f"✅ 已移除第{episode}集 分镜 {args.shot} 的第 {idx} 条台词:")
