...
| 命令 | 功能 | 示例 |
|------|------|------|
| `python .agents/skills/create-video-script/scripts/add_stage.py` | 添加一条场景定义 | `python .agents/skills/create-video-script/scripts/add_stage.py -p 古人在现代 -e 1 1 "现代商场/中央扶梯" "陈书文" "图像1为背景：..."` |
| `python .agents/skills/create-video-script/scripts/add_stage.py` | 直接引用基础场景（无角色/无修改） | `python .agents/skills/create-video-script/scripts/add_stage.py -p 古人在现代 -e 1 1 "现代商场/中央扶梯" "" ""` |
...
1. `分镜序号` — 整数，从 1 开始
2. `基础场景标签` — **必填**，场景引用如 `现代商场/中央扶梯`（需与 `stage/` 下的资产文件路径一致），或关键字 `prev`（同集上一分镜最后场景，**仅直接引用**）
...
脚本会自动校验：
- `基础场景` 非空；普通引用时场景资产文件（`design/prompt/stage/{场景}/{子场景标签}.md`）存在；`prev` 时当前分镜 > 1 且上一分镜 `stage.json` 非空
