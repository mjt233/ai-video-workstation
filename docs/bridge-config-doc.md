...
| `text-generation` | 文本生成 | 文本 → **文本文件**（产物不落 assert/，见下） |

**类型标签优先级**：按上表顺序匹配（靠前命中优先），`text-generation` 优先级最低——同时打了媒体类型标签的工作流仍按媒体类型注册，避免误判成文本。

**文本生成**（`text-generation`）：无需尺寸/时长等结构性字段，工作站只提交提示词与可选 `seed`
（外加 `expose_field` 暴露的用户参数）。**产物必须是文本文件**：工作流把结果写成 `.txt` / `.md` /
`.json` 输出文件，工作站按扩展名从 output-files 识别后下载解码为文本（≤1MB，BOM 自动剥离），
写回画布「文本生成」节点的 `config.output`；`.json` 按原文读入（需要提取字段时请让工作流直接输出纯文本）。
画布节点连接的图片等素材**当前不上传**给 Bridge（文本工作流参数约定未固定，先打通纯文本链路）。
