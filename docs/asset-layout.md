...
- 已生成音频的声音变体在资产选择器「角色」页签的「音色」分区下可选（`{角色名}/音色/{变体id}`），可被资产画布「加载音频」节点读取（`config.assetPath`），也可从资产浏览器拖拽角色到资产画布的拖放菜单音频分组中选择
...
- 分镜引用：`{场景名}/{子场景标签}@{变体id}`
- 已生成图片的变体在资产选择器「场景」页签可选，也可从资产浏览器拖拽子场景到资产画布的拖放菜单中选择
...
**回收站**（`design/.trash/`，**系统全局、跨项目共享**，与项目目录同级）：

```
design/
├── {项目A}/assert/custom/...            # 被清理文件的原始位置
└── .trash/                              # 全局回收站（系统保留目录，不参与项目列表）
    └── 20260908-201926/                 # 批次号 YYYYMMDD-HHmmss（一次清理一个批次）
        └── {项目A}/custom/canvas/x.png  # 原 assert/ 内相对路径
```

- 原位置 = `design/{项目名}/assert/{回收站内相对路径}`；恢复即反向 rename（目标已存在同名文件则跳过，不覆盖）
- 条目时间取**批次号时间**（即移入回收站时间），保留期从移入时刻起算
- `.trash` 为保留目录：`GET /api/projects` 不列出，创建/删除/导入同名项目被拒绝

**存储清理扫描**（项目设置面板 →「存储清理」页签）：

| 类型 | 口径 |
|------|------|
| 无引用自定义资产 | `assert/custom/**` 下未被项目任何文本文件引用的文件（引用来源：`prompt/**` 全文匹配 `custom/...`，含 `stage.json`、`canvas.json`、`refs.json`、变体元数据；保守策略，宁可漏删不误删） |
| 久远历史记录 | 画布节点产物历史 + 角色/场景/道具及其衍生变体历史，且归档时间早于阈值（默认 7 天，可配置）；**不含**分镜产物历史（`assert/scene/*​/*​/{stage,voice,video}/history/**`） |

- 阈值（天）由用户输入，默认 7；仅作用于历史记录，无引用自定义资产不受阈值影响
- 结果按分组展示路径 / 大小 / 时间 / 所属资产，支持预览、下载、全选 / 全不选 / 反选，并显示总大小与已选总大小
- 清理动作 = 移入全局回收站（移入前服务端**二次校验引用**，仍被引用的资产跳过）

**定时自动清理**（系统设置 → 系统设置 → 回收站）：

- 配置落盘 `server/config/system.json`：`{ trash: { autoClean: { enabled, intervalDays, retentionDays }, lastRunAt } }`，默认启用、间隔 7 天、保留期 7 天
- 服务端调度器每 1 小时轮询一次，距上次执行达到间隔即清理回收站中**超过保留期**的条目；启动后 60 秒补跑一次
- 触发与结果打印 `[trash-auto-clean]` 控制台日志（禁用状态仅在状态变化时打印一次，避免轮询刷屏）
- 回收站的查看 / 恢复 / 彻底删除 / 清空 / 立即清理一次，均在「系统设置 → 系统设置 → 回收站」子类中完成
...
存储清理（见 3.0.3）另有一套保护：移入回收站前对 `assert/custom/**` 条目**重新做一次引用检查**，仍被引用的资产跳过并在响应中给出原因；回收站条目恢复时若原位置已有同名文件则跳过（不覆盖）。
