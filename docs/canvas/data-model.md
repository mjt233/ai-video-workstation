...
- **几何派生的直接后果（含分组的复制粘贴必须避让）**：既然成员关系只由重叠决定，**复制出的分组副本绝不能与原件矩形重叠**（且需留净距，见 T9）——否则两个分组会同时“认领”对方的节点，拖动任一方都会把对方节点带走。落点由 `pastePlacement.ts: clipboardPlacementOffset` 统一计算（首选原内容右下方向外一个身位、被占用则逐级探测），实现与约束见 [interactions.md](./interactions.md) 约束 T9；**载荷不含分组（纯节点复制）时不避让，沿用历史固定 30px 偏移**。
...
- **输入转发节点（`forward-input`）没有自身产物文件**：其"当前结果"就是**上游来源的产物**——`getNodeCurrentAssetPath` 穿透整条转发链返回上游路径，故画布加载时对转发节点探测的其实是上游产物的存在性/mtime（`nodeOutputs` 里该节点的 `path` 指向上游），下游 `collectInputs` 收到的 `nodeId/path/label` 也都是**最终上游节点**。因此：转发节点不需要（也不应）创建 `assert/{scope}/canvas/{forwardNodeId}/` 目录，`config.current`/`config.outputExt` 等产物镜像字段对它无意义。
