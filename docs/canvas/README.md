...
| [notification.md](./notification.md) | **工作流完成通知气泡**：触发来源（统一任务广播）、30s 固定自动关闭、产物预览与放大、失败卡、与任务管理器抽屉的分工 |
...
系统以**文件系统即数据库**：画布定义（节点、连线、坐标、配置）持久化为 `prompt/` 下的 `canvas.json`；生成产物为 `assert/` 下的图片文件（**例外**：`text-generate`（文本生成）节点的产物是文本，写在节点 `config.output` 里，不落 `assert/`）。
...
- **持久分组的成员关系由几何实时派生**（节点矩形与分组矩形重叠面积 > 0 即成员）。由此推出一条硬约束：**含分组的复制/粘贴/插入内容，其包围盒必须与画布已有节点、分组矩形零重叠且留有净距（默认 30px）**（落点算法见 `canvas/pastePlacement.ts`，约束说明见 [interactions.md](./interactions.md) 的 T9）——否则副本会与原件互相“抓走”对方节点。**纯节点载荷（不含分组）不受此约束**：仍按历史固定 30px 偏移粘贴。
- 画布定义 `canvas.json` 为纯前端数据，服务端只把它当作普通 `prompt/` 下文件读写，不参与工作流；生成时才通过既有工作流 API 提交任务。
- **输入转发节点**（`forward-input`）：收到什么输入就输出什么（支持任意类型、可多路），自身不落盘——输出类型/资产/文本都按连线穿透到**最终上游来源**，用于把同一份产物分发给多个下游或先占位后接来源。输出类型有三种状态：具体类型（单一来源）、类型并集（混合来源，徽标显示「任意媒体」）、尚未确定（未接输入，徽标显示「任意类型」且连线校验放行任意下游）。详见 [node-types.md](./node-types.md) 与 [development.md](./development.md) 的 `passThrough` 约定。
