...
- **持久分组的成员关系由几何实时派生**（节点矩形与分组矩形重叠面积 > 0 即成员）。由此推出一条硬约束：**复制/粘贴/插入产生的新内容，其包围盒必须与画布已有节点、分组矩形零重叠且留有净距（默认 30px）**（落点算法见 `canvas/pastePlacement.ts`，约束说明见 [interactions.md](./interactions.md) 的 T9）——否则副本会与原件互相“抓走”对方节点。
