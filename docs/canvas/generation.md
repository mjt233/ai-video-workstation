...
5. 运行中任务状态来源与**画布恢复 Loading**（见 [task-architecture.md](./task-architecture.md)）：提交时随任务携带**画布定位**（`nodeId` + `canvas`），供画布加载/切换/刷新后恢复节点加载态。
   - **工作流任务**（图片/视频/TTS 生成）：`params.nodeId` / `params.canvas` 随任务 params 持久化（SQLite），引擎开始执行时透传登记进统一注册表；本地轮询 `GET /api/workflow/tasks/:id`（引擎为权威，含阶段日志）。
   - **ffmpeg 任务**（拼接 / 裁剪视频 / 裁剪音频 / 取帧）：**不使用 localStorage**——提交即拿到 `taskId`，进度与终态由服务端统一任务注册表经 WS 广播驱动（`taskSocket.onTaskUpdate`）。
   - `restore(knownNodeIds)`（画布加载与 `switchTarget` 时调用）按「项目 + 画布 scope + 节点仍在画布上」恢复 loading：注册表（WS 全量快照；快照未就绪时 HTTP 兜底 `GET /api/tasks`）覆盖 ffmpeg 与已登记的工作流任务，并**补查 SQLite 工作流任务的 pending/running**（引擎 2s tick 排队窗口与服务重启期间注册表为空）；恢复后工作流任务续跑轮询、ffmpeg 任务重订阅广播。**任务未到终态前 loading 一直保持，终态收敛时经 `onResult` 刷新产物。**
