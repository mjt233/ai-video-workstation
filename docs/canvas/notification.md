...
可直接预览产物、点开放大、`×` 手动关闭，**30 秒后自动消失**（同时最多 3 张，超出先淘汰最旧的一张）。
要看历史记录（含更早的任务、失败原因、日志）去**任务管理器抽屉 →「历史」页签**（见 [../task-manager/ui.md](../task-manager/ui.md)）。
...
| 文本产物（无文件） | 浅底说明条：「文本产物已完成，可在画布节点或「任务管理器 → 历史 → 任务详情」查看」 | — |
