...
- **全局抽屉**：Header 右上角独立图标（`mdi-progress-clock` + 活跃任务数徽标）→ `TaskManagerDrawer.vue`（**右侧抽屉**版统一任务管理器，覆盖工作流/LLM/ffmpeg 全部异步任务；节点名/模型名/阶段/进度/耗时（按 `startedAt` 客户端每秒刷新）/画布位置；每行「中断」（取消非删除，无需 confirm，不可中断的任务置灰并显示原因）；「历史」页签含产物缩略图与放大预览）。工作流任务完成时另有右下角**完成通知气泡**（`canvas/notify.ts` + `WorkflowNotifyStack.vue`），见 [notification.md](./notification.md)。**架构与实现见 [task-architecture.md](./task-architecture.md)**。
