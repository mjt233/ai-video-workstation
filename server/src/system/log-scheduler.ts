 ...
 * 任务日志自动清理调度器。
 *
 * 触发规则（与回收站调度器 `trash-scheduler.ts` 同一套形态，仅间隔粒度不同）：
 * - 每 `CHECK_INTERVAL_MS`（默认 1 小时）轮询一次配置，判断距上次执行是否已达到
 *   配置的执行间隔（默认 24 小时）——到达即执行一轮自动清理；
 ...
 *   无需重启服务；
 * - 进程内重入保护：上一轮未结束时跳过本轮（`VACUUM` 会长时间持锁，必须避免并发）。
 *
 * 日志前缀统一为 `[tasklog-auto-clean]`，仅在「状态变化」「真正触发」「执行失败」时打印，
 * 空闲轮询不产生日志（避免每小时刷屏）。
 */
import { readSystemSettings, SYSTEM_SETTINGS_PATH } from './system-settings.js';
import { runTaskLogAutoClean, shouldRunLogAutoClean } from './log-cleaner.js';
...
async function readSettingsAndLogState(configPath: string): Promise<{
  ...
  const { enabled, intervalHours, retentionDays } = settings.taskLog.autoClean;
  if (state.lastEnabled !== enabled) {
    state.lastEnabled = enabled;
    if (enabled) {
      console.log(
        `[tasklog-auto-clean] 已启用：每 ${intervalHours} 小时执行一次，日志保留期 ${retentionDays} 天`,
      );
    } else {
      console.log('[tasklog-auto-clean] 已禁用（可在 系统设置 → 系统设置 → 日志 中开启）');
    }
  }
  return { enabled, intervalHours, retentionDays, lastRunAt: settings.taskLog.lastRunAt };
}
...
async function tick(reason: 'scheduled' | 'startup', configPath: string): Promise<void> {
  if (state.running) {
    console.log('[tasklog-auto-clean] 上一轮清理尚未结束，跳过本轮');
    return;
  }
  state.running = true;
  try {
    const { enabled, intervalHours, lastRunAt } = await readSettingsAndLogState(configPath);
    if (!enabled) return;
    const now = new Date();
    if (!shouldRunLogAutoClean(lastRunAt, intervalHours, now)) return;
    if (reason === 'startup') {
      console.log(
        `[tasklog-auto-clean] 检测到已超期（上次执行 ${lastRunAt ?? '从未执行'}），立即补跑一次`,
      );
    }
    await runTaskLogAutoClean({ reason, configPath, now });
  } catch (e) {
    // 自动清理失败不影响服务运行：打印完整错误，不更新 lastRunAt（下轮自动重试）
    console.error('[tasklog-auto-clean] 执行失败:', e);
  } finally {
    state.running = false;
  }
}
 ...
 */
export function startTaskLogCleanScheduler(
  options: {
    configPath?: string;
 ...
 */
export function stopTaskLogCleanScheduler(): void {
...
/**
 * 当前调度器是否正在执行一轮清理（供手动清理避免与自动清理并发）。
 *
 * @returns 正在执行返回 true
 */
export function isTaskLogCleanRunning(): boolean {
