...
/**
 * 画布节点固定产物路径（生成图片 output.jpg / 图片修剪与扩展 output.png / 生成视频 output.mp4）：
 * - 分镜画布：assert/scene/{集数}/{分镜}/canvas/{nodeId}/output.{ext}
 * - 场景画布：assert/stage/{场景名}/canvas/{子场景标签}/{nodeId}/output.{ext}
 *
 * 注：png 为「图片修剪与扩展」节点（纯前端 canvas 生成 + 上传落盘，输出格式可选 png/jpg）
 * 的产物扩展名；该节点无「上传产物」按钮，但仍需通过本端点落盘（服务端负责归档旧产物）。
 */
const CANVAS_OUTPUT_PATH =
  /^assert\/(?:scene\/[1-9]\d*\/[1-9]\d*\/canvas\/[^/]+|stage\/[^/]+\/canvas\/[^/]+\/[^/]+)\/output\.(jpg|png|mp4)$/u;

/** 允许上传到图片产物（output.jpg / output.png）的 MIME（与 /assets/upload 一致） */
 ...
 * @returns 小写扩展名（如 jpg / png / mp4），无扩展名时为空串
 */
...
/**
 * 校验目标路径为画布节点固定产物路径（output.jpg / output.png / output.mp4）并返回规范化路径。
...
export function assertCanvasNodeOutputPath(relPath: string): string {
  ...
  if (!normalized || normalized.includes('..') || !CANVAS_OUTPUT_PATH.test(normalized)) {
    throw Object.assign(
      new Error('仅支持上传到画布节点的固定产物路径（output.jpg / output.png / output.mp4）'),
      { code: 'INVALID' },
    );
  }
  return normalized;
...
export function assertCanvasUploadFile(targetExt: string, mime: string, originalName: string): void {
  ...
  if (targetExt === 'png') {
    if (mime !== 'image/png' && extOf(originalName) !== 'png') {
      throw Object.assign(new Error('PNG 产物仅支持上传 png 格式图片'), { code: 'INVALID' });
    }
    return;
  }
...
export async function saveCanvasNodeUpload(
  ...
  // 目标扩展名按路径后缀判定（正则已保证只会是 jpg / png / mp4 之一）
  const targetExt = rel.endsWith('.jpg') ? 'jpg' : rel.endsWith('.png') ? 'png' : 'mp4';
