...
- 分镜 `stage.json` 中 `prompt` 字段是否正确使用"图像1"代表基础场景、"图像2"代表第一个角色、"图像3"代表第二个角色。不应对基础场景和人物外貌设计中已有的描述重复。（如：人物外貌设计中已经说明了人物穿了什么衣服，拿着什么东西，那么 `stage.json`的`prompt` 中就不要重复这部分描述）
