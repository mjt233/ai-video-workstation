...
> **约束：**
> - `基础场景` **必须非空**，且值必须在 `design/prompt/stage/{场景}/{完整场景标签}.md` 中存在对应的资产文件
> - `登场角色` 与 `prompt` **同时为空**时，表示**直接引用基础场景图**，不做任何修改。生成分镜场景图时，由**工作流调度引擎**将 `assert/stage/{场景}/{标签}.jpg` **复制**为独立资产 `assert/scene/{集}/{镜}/stage/{index}.jpg`（不调用图像编辑）
> - 需要合成/修改画面时：`prompt` 必须非空；若有 `登场角色`，须在 `design/prompt/character/{角色名}` 下存在资产文件，数组长度不超过 2
