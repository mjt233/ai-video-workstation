...
参数:
    分镜序号        - 整数，从 1 开始
    基础场景标签    - 必填，场景完整标签，如 "现代商场/现代商场-白天-平视-晴-中央扶梯"
    角色名          - 逗号分隔的角色名，最多2个；传空字符串 "" 表示无登场角色
    prompt          - 组合提示词；与角色同时为空时表示直接引用基础场景
    -e, --episode   - 集数（默认: 1）

示例:
    python scripts/add_stage.py -e 1 1 "现代商场/现代商场-白天-平视-晴-中央扶梯" "陈书文" "图像1为背景：现代商场中央扶梯；图像2站在中央扶梯右侧，身体朝向扶梯，面部微侧向镜头，驻足张望。"
    python scripts/add_stage.py -e 1 2 "现代商场/现代商场-白天-平视-晴-奶茶店前" "陈书文,现代女孩" "图像1为背景：奶茶店前；图像2站在柜台前偏左，身体朝向图像3，面部朝向图像3；图像3站在图像2右侧约一步、靠近柜台，身体朝向图像2，面部朝向图像2。"
    python scripts/add_stage.py -e 1 3 "现代商场/现代商场-白天-平视-晴-中央扶梯" "" ""
"""
...
def parse_args():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
  %(prog)s -e 1 1 "现代商场/现代商场-白天-平视-晴-中央扶梯" "陈书文" "图像1为背景..."
  %(prog)s -e 1 2 "现代商场/现代商场-白天-平视-晴-奶茶店前" "陈书文,现代女孩" "图像1..."
  %(prog)s -e 1 3 "现代商场/现代商场-白天-平视-晴-中央扶梯" "" ""
        """,
    )
    parser.add_argument("shot", type=int, help="分镜序号（从 1 开始）")
    parser.add_argument("stage_ref", type=str, help="基础场景完整标签（必填），如 现代商场/现代商场-白天-平视-晴-中央扶梯")
    parser.add_argument("characters", type=str, help="登场角色名，多个用逗号分隔（最多2个）；空字符串表示无角色")
    parser.add_argument("prompt", type=str, help="组合提示词；与角色同时为空时表示直接引用基础场景")
...
def main():
    ...
    if len(characters) > 0 and not prompt.strip():
        print("❌ 有登场角色时 prompt 不能为空。", file=sys.stderr)
        sys.exit(1)

    # 校验场景资产是否存在
    if not validate_stage_ref(stage_ref, project_name, project_root):
